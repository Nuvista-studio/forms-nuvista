<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-lg font-bold" style="color: var(--color-text-primary);">Form Pengembalian Asset</h1>
            <p class="text-sm mt-1" style="color: var(--color-text-secondary);">Kembalikan asset yang masih terpasang sebelum user diubah statusnya menjadi Resigned.</p>
        </div>
    ...
    <form wire:submit="submit" class="space-y-5">
        ...
        <div class="glass-card p-5">
            <h2 class="text-sm font-semibold mb-3" style="color: var(--color-text-primary);">Dibuat Oleh (Teknisi)</h2>
        ...
        {{-- Pilih User --}}
        <div class="glass-card p-5">
            <h2 class="text-sm font-semibold mb-3" style="color: var(--color-text-primary);">User yang Mengembalikan Asset</h2>
                    ...
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p class="text-xs mt-0.5" style="color: var(--color-text-secondary);">
                                {{ $penggunaNik ? 'NIK ' . $penggunaNik . ' • ' : '' }}{{ $penggunaDepartment ?: 'Tanpa department' }}
                            </p>
                        </div>
            ...
            @else
                <div class="relative">
                    <input wire:model.live="penggunaSearch" wire:keyup="searchPengguna" type="text" placeholder="Cari nama, NIK, atau email..."
                            ...
                            @foreach ($penggunaResults as $result)
                                <button type="button" wire:click="selectPengguna({{ $result['id'] }})"
                                    ...
                                    <span class="block text-xs" style="color: var(--color-text-secondary);">
                                        {{ $result['nik'] ? 'NIK ' . $result['nik'] . ' • ' : '' }}{{ $result['department'] ?? 'Tanpa department' }}
                                    </span>
                                </button>
                            @endforeach
                        </div>
                ...
                <p class="text-xs mt-2" style="color: var(--color-text-secondary);">Hanya user berstatus Active yang bisa dipilih.</p>
        ...
        @if ($penggunaId)
            <div class="glass-card p-5">
                <h2 class="text-sm font-semibold mb-1" style="color: var(--color-text-primary);">Asset Terpasang</h2>
                <p class="text-xs mb-3" style="color: var(--color-text-secondary);">Centang asset yang dikembalikan. Asset akan di-unassign dan statusnya menjadi inactive.</p>

                @if (count($availableAssets) === 0)
                    <div class="p-3 rounded-lg text-sm"
                        style="background: rgba(34, 197, 94, 0.12); border: 1px solid rgba(34, 197, 94, 0.3); color: #22c55e;">
                        User ini tidak memiliki asset terpasang. Siap untuk di-resign.
                    </div>
                @else
                    <div class="space-y-2">
        ...
        {{-- Detail Pengembalian --}}
        <div class="glass-card p-5">
            <h2 class="text-sm font-semibold mb-3" style="color: var(--color-text-primary);">Detail Pengembalian</h2>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label class="text-xs text-muted">Tanggal Pengembalian</label>
                ...
                <div>
                    <label class="text-xs text-muted">Kondisi Asset</label>
                ...
                <div>
                    <label class="text-xs text-muted">Kelengkapan</label>
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" wire:loading.attr="disabled"
                ...
                <span wire:loading.remove wire:target="submit">Simpan Pengembalian</span>
