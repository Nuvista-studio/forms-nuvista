<div class="space-y-6" x-data="columnManager()" x-init="init(['status','nomor_form','teknisi','pengguna','perangkat','site','kondisi','tanggal','aksi'])" x-on:form-deleted.window="$wire.$refresh()">
    {{-- Header --}}
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b" style="border-color: var(--color-border);">
                            ...
                            {{-- Kondisi --}}
                            <th x-show="isVisible('kondisi')" class="px-4 py-3 text-left text-xs font-medium text-muted uppercase tracking-wider">Kondisi</th>
                            {{-- Tanggal --}}
                    ...
                    <tbody class="divide-y" style="border-color: var(--color-border);">
                        @foreach($forms as $form)
                            <tr class="transition-colors duration-150" style="hover: background: var(--color-glass-bg);">
                                ...
                                {{-- Kondisi --}}
                                <td x-show="isVisible('kondisi')" class="px-4 py-3">
...
function columnManager() {
    return {
        columns: [],
        init(defaults) {
            const stored = localStorage.getItem('pemeriksaan_columns');
            if (stored) {
        ...
        defaultColumns(keys) {
            const labels = {
                ...
                kondisi: 'Kondisi', tanggal: 'Tanggal', aksi: 'Aksi'
            };
        ...
        startDrag(idx, event) {
            ...
            const onMove = (e) => {
                ...
                if (target) {
                    dragEl.parentNode.insertBefore(dragEl, target);
                } else {
                    const last = items[items.length - 1];
                    if (last && last !== dragEl) dragEl.parentNode.appendChild(dragEl);
                }
            };
            ...
            const onUp = () => {
                ...
                clone.remove();
                // Update columns array based on DOM order
                const items = [...dragEl.closest('div').querySelectorAll('label')];
                const newOrder = items.map(label => {
                    const key = label.querySelector('input[type="checkbox"]')?.getAttribute('x-model')?.replace('columns[', '').replace('].visible', '');
                    const idx = parseInt(label.querySelector('span:last-child')?.textContent) - 1;
                    return idx;
                });
                // Re-sort columns array
                const sorted = newOrder.map(i => this.columns[i]).filter(Boolean);
                if (sorted.length === this.columns.length) {
                    this.columns = sorted;
                }
                this.save();
            };
        ...
        save() {
            localStorage.setItem('pemeriksaan_columns', JSON.stringify(this.columns));
        },
        resetColumns() {
            localStorage.removeItem('pemeriksaan_columns');
            this.columns = this.defaultColumns(['status','nomor_form','teknisi','pengguna','perangkat','site','kondisi','tanggal','aksi']);
        }
    };
}
</script>
