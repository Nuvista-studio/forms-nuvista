                    ...
                    <div class="pt-4">
                        <div class="mb-4">
                            ...
                            <div class="mt-2 flex flex-col sm:flex-row gap-2">
                                <div id="qr-reader-pwt" class="w-full rounded-lg overflow-hidden" style="display: none; min-height: 200px;"></div>
                                <button type="button"
                                    @click="if(!scanning) { startScannerPwt(); scanning = true; } else { stopScannerPwt(); scanning = false; }"
                                    :class="scanning ? 'bg-red-500 text-white' : ''"
                                    class="glass-button-secondary text-sm flex items-center gap-1.5 w-fit">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2 mt-1">
                                <input type="text" wire:model.live="noAsset" placeholder="Contoh: ASR-2024-001" class="glass-input flex-1 rounded-lg px-3 py-2 text-sm" @keydown.enter.prevent="$wire.searchAssetManual()">
                                <button wire:click="searchAssetManual" type="button" class="glass-button-secondary text-sm px-3">Cari</button>
                            </div>
                        </div>
                        @if($assetId)
                        ...
                        <div class="flex justify-between pt-4">
                            ...
                            <button wire:click="nextStep" type="button" class="glass-button-primary text-sm" {{ !$assetId ? 'disabled style="opacity:0.5;cursor:not-allowed;"' : '' }}>Selanjutnya →</button>
                        </div>
                    </div>
                </div>
            </div>
...
let html5QrCodePwt = null;

function startScannerPwt() {
    const el = document.getElementById('qr-reader-pwt');
    if (el) el.style.display = 'block';

    if (!html5QrCodePwt) {
        html5QrCodePwt = new Html5Qrcode('qr-reader-pwt');
    }

    html5QrCodePwt.start(
...
function stopScannerPwt() {
    if (html5QrCodePwt && html5QrCodePwt.isScanning) {
        html5QrCodePwt.stop().then(() => {
            const el = document.getElementById('qr-reader-pwt');
